...
class KuliahInformatikaFlow(FlowSpec):
    ...
    def mengerjakan_tugas(self):
        ...
        self.nilai_tugas = {mk: 80 + (i * 5) for i, mk in enumerate(self.matakuliah)}  
        print(f"Nilai tugas: {self.nilai_tugas}")
        self.next(self.ujian_akhir)

    @step
    def ujian_akhir(self):
        print("Langkah 5: Mengikuti ujian ak[hir.")
        self.nilai_ujian = {mk: 70 + (i * 10) for i, mk in enumerate(self.matakuliah)}  
